...
**版本**: V1.0.0 | **日期**: 2026-06-11 | **作者**: maximilian
...
**可变长度**（6–42 字节），所有多字节字段为**小端序 (Little-Endian)**。
...
| 1 | 1 | `cmd` | 命令码（见 §4） |
| 2 | 1 | `flags` | 标志位（见 §5） |
| 3 | 1 | `datalen` | data 段实际字节数 (0–36, 见 §6) |
| 4 | N | `data[]` | 电机数据 (N = datalen, 见 §6) |
| 4+N | 1 | `crc_check` | XOR 校验 (覆盖 cmd, flags, datalen, data[0..N-1]) |
| 5+N | 1 | `ender` | 帧尾，固定值 `'e'` (0x65) |

**帧长度规则**: `frame_bytes = 6 + datalen` (最小 6, 最大 42)

**每电机字节数**: 普通命令 2 字节/电机, SET_POSITION_SPEED 为 4 字节/电机
...
2. `datalen` 必须 ≤ 36, 且满足对应 CMD 的每电机字节数整除
3. `ender` 必须等于 `'e'` (0x65)
4. CAN-FD DLC 对应字节数必须等于 `6 + datalen`
5. `crc_check` 必须等于 `cmd ^ flags ^ datalen ^ data[0] ^ ... ^ data[datalen-1]` (XOR)
6. 以上任一条件不满足，帧将被丢弃
...
**控制应答** — CMD 与原始控制命令相同 (0x01–0x06):

| data[2×id] | 格式 | 说明 |
|------------|------|------|
| `uint16 LE` | 错误码 | `0x0000` = 成功 (MOTOR_CONTROL_OK) |
...
data 段最多 36 字节，按电机 ID 编址。**实际使用的字节数由 `datalen` 指定**。

**普通命令 (2 字节/电机, CMD 0x01-0x06, 0x10-0x13)**:

| 电机 ID | data 偏移 | 说明 |
|---------|----------|------|
| 1 | `data[0..1]` | 低字节+高字节, uint16 LE |
| N | `data[(N-1)*2..(N-1)*2+1]` | datalen ≤ 18 |

**SET_POSITION_SPEED (4 字节/电机, CMD 0x07)**:

| 电机 ID | data 偏移 | 说明 |
|---------|----------|------|
| 1 | `data[0..1]`=pos, `data[2..3]`=speed | uint16 LE + int16 LE |
| N | `data[(N-1)*4..(N-1)*4+3]` | datalen ≤ 36 |
...
| 22 | crc_check | `0x01` | XOR(cmd..data[17]) |
| 23 | ender | `0x65` | `'e'` |

```
完整帧: 73 01 00 12 B8 0B B8 0B B8 0B B8 0B B8 0B B8 0B B8 0B B8 0B B8 0B 01 65
```

### 9.2 设置电机1位置 90.0° (仅控制1个电机)

**CAN ID**: 0x100, **datalen**: 2, **帧长**: 8 bytes
...
| 2 | flags | `0x01` | 需要应答 |
| 3 | datalen | `0x02` | 2 (仅电机1) |
| 4–5 | data[0..1] | `84 03` | 900 × 10⁻¹° = 0x0384 (LE) |
| 6 | crc_check | `0x8A` | 0x02^0x01^0x02^0x84^0x03 |
| 7 | ender | `0x65` | `'e'` |

```
完整帧: 73 02 01 02 84 03 8A 65
```
...
| Value | `0x73` | `0x10` | `0x00` | `0x00` | `0x10` | `0x65` |

crc_check = 0x10 ^ 0x00 ^ 0x00 = 0x10
...
2. **可变帧长**: `datalen` 字段支持按需传输 — 仅控制 1 个电机时帧长 8 字节 (vs 固定 42 字节)，节省带宽。

3. **简洁帧结构**: 仅 7 个字段，无复杂的嵌套 TLV 或可变长度段。解析器通过 `datalen` 直接计算帧尾位置，O(1) 定位。

4. **硬件 CRC + 软件 XOR 双重保护**: CAN-FD 硬件 CRC 保证链路层完整性，协议层 XOR 校验覆盖 cmd→data 段，防御软件编解码错误。
